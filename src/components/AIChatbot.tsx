...
import { api, getApiErrorMessage, renderText } from '@/lib/api';
...
export default function AIChatbot() {
  ...
  const [chats, setChats] = useState<{ role: 'ai' | 'user'; text: string }[]>([
    { role: 'ai', text: 'Hi! I am the PropShare AI Assistant. How can I help you with your investment journey today?' }
  ]);
    ...
    <div className="fixed bottom-6 right-6 z-50">
      ...
      {isOpen && (
        <div className="chatbot-window absolute bottom-20 right-0 w-[350px] md:w-[400px] h-[500px] bg-card border border-white/10 rounded-2xl shadow-3xl flex flex-col overflow-hidden backdrop-blur-xl">
              ...
              <div key={idx} className={`flex ${chat.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                ...
                >
                  {typeof chat.text === 'object' && chat.text !== null 
                    ? (chat.text as any).text || JSON.stringify(chat.text) 
                    : String(chat.text)}
                </div>
              </div>
            ))}
          ...
          {/* Input Area */}
          <form onSubmit={handleSend} className="p-4 bg-background border-t border-white/5 relative">
            <input
              type="text"
              value={message}
